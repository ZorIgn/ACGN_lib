...
- 🧭 **个性化发现** —— 搜索框下根据书架、评分和作品资料推荐，结合公开热度与题材多样性排序；最多 50 部、每页 12 部，可切换热门作品。
...
每张卡片附有推荐理由，资料源评分与自己的星级评分分开展示。推荐最多 50 部，每页 12 部，可翻页浏览。点击「加入清单」后填写个人记录，保存草稿或导入后返回进入前的页面，保留作品类型、推荐模式与页码。推荐单独加载，资料源暂时不可用时仍可继续搜索。

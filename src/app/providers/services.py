...
redis_db = None if getattr(settings, "NATIVE_RUNTIME", False) else get_redis_client()
bucket_key = f"{settings.REDIS_PREFIX}_api" if settings.REDIS_PREFIX else "api"

session = (
    LimiterSession(per_second=5)
    if redis_db is None
    else LimiterSession(
        per_second=5,
...
def get_media_metadata(
    ...
    if source == Sources.BANGUMI.value:
        return bangumi.metadata(media_id, media_type)
    if source == Sources.STEAM.value:
        return steam.metadata(media_id)
    if source == Sources.WEBNOVEL.value:
        return webnovel.metadata(media_id)
